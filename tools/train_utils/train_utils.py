...
#             loss, pred, targets = model_func(model, batch)
#             val_loss += loss.item()
...
#     model.train()  # validation 후 다시 학습 모드로 전환

#     return val_loss, val_accuracy


# 24.10.31 - train model 원본 - jw
...
#             # validation 수행
#             val_loss, val_accuracy = validate_model(model, val_loader, model_func, logger)

#             # TensorBoard에 validation 결과 기록
#             if tb_log is not None:
#                 tb_log.add_scalar('val/loss', val_loss, cur_epoch)
#                 tb_log.add_scalar('val/accuracy', val_accuracy, cur_epoch)
...
#                 ckpt_name = ckpt_save_dir / ('checkpoint_epoch_%d' % (cur_epoch + 1))
#                 save_checkpoint(
