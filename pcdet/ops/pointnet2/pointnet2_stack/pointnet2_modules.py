...
# modified by hoon(10-28) -> num_reduced_channels = 2로 바꿈 (origin : 30)
class VectorPoolAggregationModule(nn.Module):
    ...
    def forward(self, xyz, xyz_batch_cnt, new_xyz, new_xyz_batch_cnt, features, **kwargs):
        ...
        features = features.view(N, -1, self.num_reduced_channels).sum(dim=1) # N, self.num_reduced_channels
        # print(f'features야 어떻게 나오니 : {features}')
