...
class PointFeatureEncoder(object):
    ...
    def absolute_coordinates_encoding(self, points=None):
        ...
        # print("len(self.src_feature_list):", len(self.src_feature_list))    
        assert points.shape[-1] == len(self.src_feature_list)
        # point_feature_list = [points[:, 0:3]] -- origin
        point_feature_list = [points[:, 0:4]] # -- 24.10.28 modified by jw 
        for x in self.used_feature_list:
            if x in ['x', 'y', 'z', 'intensity']: # -- 24.10.28 modified by jw , ['x', 'y', 'z']가 origin
                continue
