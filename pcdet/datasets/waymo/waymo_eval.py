        ...
        for frame_index, info in enumerate(infos):
            if is_gt:
                ...
                if 'num_points_in_gt' in info:
                    ...
                    # print(type(zero_difficulty_mask))
                    # print(type(info['num_points_in_gt'] > 5))
                    ...
                    info['difficulty'] = [1 if num > 5 else 2 for num in info['num_points_in_gt']]
                    # nonzero_mask = info['num_points_in_gt'] > 0
                    nonzero_mask = [num > 0 for num in info['num_points_in_gt']]
                ...
                # difficulty를 numpy 배열로 변환 후 마스크 적용
                difficulty_np = np.array(info['difficulty'])
                difficulty.append(difficulty_np[box_mask])
